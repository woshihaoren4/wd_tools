...
impl<T> CopyLock<T> {
    pub fn new(default: T) -> CopyLock<T> {
        ...
        let list = if let Ok(o) = (0..COPY_LOCK_LENGTH)
            .map(|_| raw.clone())
            .collect::<Vec<_>>()
            .try_into()
        {
        ...
        let list_status = (0..COPY_LOCK_LENGTH)
        ...
        F: FnOnce(Arc<T>) -> T,
    {
        ...
        unsafe {
            let arc = &mut *(&self.list as *const [Arc<T>; COPY_LOCK_LENGTH]
                as *mut [Arc<T>; COPY_LOCK_LENGTH]);
