...
where T:Fn(Ctx,Response)->F + Send+Sync,
...
impl Http{
    ...
    pub fn hook_client_build(mut self,func:impl Fn(Ctx,ClientBuilder)->anyhow::Result<Client>+Send+Sync+ 'static)->Self{
        self.client_build_hook = Some(Arc::new(func));self
    }
    pub fn hook_request_build(mut self,func:impl Fn(Ctx,RequestBuilder)->anyhow::Result<RequestBuilder>+Send+Sync+ 'static)->Self{
        self.request_build_hook = Some(Arc::new(func));self
    }
    pub fn hook_response<F:Future<Output=anyhow::Result<Box<dyn Any>>>+Sync+Send+ 'static>(mut self,func:impl Fn(Ctx,Response)->F+Send+Sync+'static)->Self{
        self.response_hook = Some(Arc::new(ResponseHookImpl{inner:func,b:PhantomData::default()}));self
    }
    ...
    pub async fn into_send<T:Any>(self)->anyhow::Result<T>{
        ...
        let client = match self.client_build_hook {
            ...
            Some(hook) => {
                hook(self.hook_ctx.clone(),builder)?
            }
        };
        ...
        if let Some(hook) =  self.response_hook{
            ...
            if (*x).type_id() != TypeId::of::<T>() {
                return Err(anyhow::anyhow!("decoding error: expect type[{:?}], but find type[{:?}]",TypeId::of::<T>(),x.type_id()))
            }
...
mod test{
    ...
    async fn test_http_into_send(){
        let http:Response = Http::new(Method::GET,"https://www.baidu.com").unwrap().header("hello","world").into_send().await.unwrap();
